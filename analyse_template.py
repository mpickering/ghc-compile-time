...
print("Total sizes: {0:.2f}s".format(total_time / 1000))
...
#    nx.draw_networkx_nodes(G, pos, ax=network_ax, nodelist=waiting, node_color="red", alpha=0.3, **options)
#    nx.draw_networkx_nodes(G, pos, ax=network_ax, nodelist=running, node_color="green", alpha=0.5, **options)
#    nx.draw_networkx_nodes(G, pos, ax=network_ax, nodelist=finished, node_color="black", alpha=0.5, **options)
#    nx.draw_networkx_edges(G, pos, ax=network_ax, width=1.0, alpha=0.2)
#    #nx.draw_networkx_labels(G, pos, font_color="red", horizontalalignment='left', font_size=8, verticalalignment="bottom")
##    plt.text(pos['SOURCE'][0],pos['SOURCE'][1] - 0.05, str(n_jobs))
##    plt.text(pos['SOURCE'][0],pos['SOURCE'][1] - 0.1, "%ds" % frames)
#
##nx.draw(G, pos, with_labels=False)
##nx.draw_networkx_labels(G, pos, font_color="red", horizontalalignment='left', font_size=8, verticalalignment="bottom")
#ani = animation.FuncAnimation(fig, update, frames=(math.ceil(events[-1][0]/1000)+10), interval=500)
##for chart_xs, chart_ys, label in charts:
##    chart_ax.plot(chart_xs, chart_ys, label=label)
##    chart_ax.legend()
